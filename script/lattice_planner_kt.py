...
'''
lattice_planner 로직에, lidar 정보를 추가하여,
장애물을 회피하도록 수정함!

또한, 기존 코드는 morai의 가상환경에서 장애물에 대한 정보를 받아서 수정했기에
<
/Object_topic 으로 들어오는 morai_msgs/ObjectStatusList 메시지가 담당
사실상 센서 데이터를 받아온건 아님,,
>

===>

실제 센서의 토픽을 받아서 구동해야 되기에 그 부분도 수정 필요!


1. lidar

2. camera -> 학습된 모델이 , people dummy 만 장애물로 인식하여 local_path에 반영
'''
