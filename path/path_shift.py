...
# 1. CSV 파일 불러오기
try:
    df = pd.read_csv('path_corrected_2.csv')
    print("원본 데이터의 일부:")
    print(df.head())
    print("\n데이터 정보:")
    df.info()
...
df.loc[start_index:end_index, 'X'] = df.loc[start_index:end_index, 'X'] + correction_amount_x
...
output_filename = 'path_corrected_3.csv'
